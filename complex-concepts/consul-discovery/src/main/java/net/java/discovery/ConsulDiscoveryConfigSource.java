...
/**
 * A service property in the form service-name.property must be mapped to a
 * system property with the name discovery.service.target-mp-config-property.
 * For example, to map URL of the pathfinder service into MP config property
 * named pathfinderurl, specify the following system property:
 * discovery.service.pathfinderurl=pathfinder.url. The system property needs to
 * be defined before the MP config property pathfinderurl is retrieved by the
 * application and then the config source provides the pathfinder service URL.
...
public class ConsulDiscoveryConfigSource implements ConfigSource {
    ...
    private static final String CONFIG_CONSUL_URL = "consul.url";
    ...
    public String getValue(String propertyName) {
        ...
        if (propertyMapping != null) {
            Consul consul = buildConsulClient();
    ...
    private static Consul buildConsulClient() {
